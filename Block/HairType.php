...
class HairType extends Template
{
    const STEP_TITLE = 'Hair Type';
    const MIAN_ATTR_NAME = 'm_hair_type';
    ...
    public function getPriceRule($product,$optionTitle,$isRemoveComma = false)
    {
        ...
        if(array_key_exists($filterOptionTitle,$prices))
        {
            ...
            if (strcmp(trim($optionTitle), 'Remy hair (best)') == 0) {
                $returnPrice = $this->_customizationHelper->getUpgradedPrice($prices);
                $optionPrice = $specialPrice * 0.4;
                if ($returnPrice) {
                    $optionPrice = ($specialPrice + $returnPrice) * 0.4;
                }
            }
            if (strcmp(str_replace('&comma;', ',', trim($optionTitle)), 'European hair (fine, thin & soft, 7" and up is not available)') == 0) {
                $returnPrice = $this->_customizationHelper->getUpgradedPrice($prices);
                $optionPrice = $specialPrice * 0.3;
                if ($returnPrice) {
                    $optionPrice = ($specialPrice + $returnPrice) * 0.3;
                }
            }
        }
        if ($isRemoveComma){
    ...
    public function getImagesByAttrName($attrName) {
        ...
        $imagesArr = array (
            array (
                'attrName' => 'Remy hair (best)',
                'attrImages' => array (
                    'mainImage' => 'hair-type2.png',
                    'thumbImage' => 'hair-type2.png',
                ),
            ),
            array (
                'attrName' => 'Indian human hair (medium thickness&comma; not avaliable for silky straight)',
                'attrImages' => array (
                    'mainImage' => 'hair-type6.png',
                    'thumbImage' => 'hair-type6.png',
                ),
            ),
            array (
                'attrName' => 'European hair (fine, thin & soft&comma; 7" and up is not available)',
                'attrImages' => array (
                    'mainImage' => 'hair-type1.png',
                    'thumbImage' => 'hair-type1.png',
                ),
            ),
            array (
                'attrName' => 'Chinese hair (coarse&comma; good for extramely straight)',
                'attrImages' => array (
                    'mainImage' => 'hair-type4.png',
                    'thumbImage' => 'hair-type4.png',
                ),
            ),
            array (
                'attrName' => 'Synthetic hair',
                'attrImages' => array (
                    'mainImage' => 'hair-type5.png',
                    'thumbImage' => 'hair-type5.png',
                ),
            )
        );
